...
def _paths_for(frame_id, kind, data_dir):
    # Frame IDs are stored as 4-digit zero-padded filenames (e.g. hen0602.fits,
    # hen1388.fits). The format must include `:04d` so IDs < 1000 resolve.
    if kind == "fitted":
        return [data_dir / f"hen{frame_id:04d}.fits"]
    if kind == "ramp":
        paths = sorted(data_dir.glob(f"hen{frame_id:04d}_*r.fits"))
        if not paths:
            raise FileNotFoundError(
                f"no ramp samples found for hen{frame_id:04d}_*r.fits in {data_dir}")
        return paths
...
def summarize(frame_ids=None, kind="fitted", log_csv=None, data_dir=None, top=10, show=True):
    ...
    if frame_ids is not None:
        ...
        for fid in ids_list:
            if kind == "fitted":
                p = data_dir / f"hen{fid:04d}.fits"
            else:
                p_list = sorted(data_dir.glob(f"hen{fid:04d}_*r.fits"))
